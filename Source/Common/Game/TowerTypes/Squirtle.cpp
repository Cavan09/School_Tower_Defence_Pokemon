...
Squirtle::Squirtle(Level* level,Tile* tile, float speed,int health, const char* textureFile) : Player(level, textureFile),
m_Texture(NULL)
{
    m_Speed = speed;
    m_Health = health;
...
void Squirtle::update(double delta)
{
    Player::update(delta);
    if(m_FireRate > 0.0)
    {
...
const char* Squirtle::getType()
{
    return MAIN_BASE_TYPE;
}


void Squirtle::handlePlayerCollision(Projectile* projectile)
{
...
void Squirtle::fireAtEnemies()
{
	...
	for( int i = 0; i < m_Level->getEnemies().size(); i++)
    {
        ...
        if(enemy != NULL && enemy->getIsActive() == true)
        {
            if(MathUtils::withinRange(enemy->getX(),getX(),350.0f) == true || MathUtils::withinRange(enemy->getY(),getY(),350.0f == true))
            {
                enemyCount++;
                ...
                if(enemyCount == 1)
                {
                    ...
                    {
                        fireProjectile(centerX, centerY,"Waterball",500.0f,2);
                    }
                }
                else
                {
...
void Squirtle::fireProjectile(float x, float y, const char* texture,float speed, int damage)
{

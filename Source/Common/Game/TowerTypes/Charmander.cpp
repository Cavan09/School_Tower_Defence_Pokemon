...
Charmander::Charmander(Level* level,Tile* tile, float speed,int health, const char* textureFile) : Player(level, textureFile),
m_Texture(NULL)
{
    m_Speed = speed;
    m_Health = health;
...
void Charmander::update(double delta)
{
    Player::update(delta);
    if(m_FireRate > 0.0)
    {
...
void Charmander::paint()
{
	Player::paint();
}
void Charmander::reset()
{
    fireAtEnemies();
    
}
const char* Charmander::getType()
{
...
void Charmander::fireProjectile(float x, float y, const char* texture,float speed, int damage)
{
